...
for ((page = 1; page <= total_pages; page++)); do
    ...
    for project in $(echo "${projects}" | jq -r '.[] | @base64'); do
        ...
        for repository in $(echo "${repositories}" | jq -r '.[] | @base64'); do
            ...
            repository_path=$(echo "${repository}" | base64 --decode | jq -r '.path')
            repository_name=$(echo "${repository}" | base64 --decode | jq -r '.name')
            if [ "$repository_name" == "$REG_DELETE" ]; then
                del_location=$(echo "${repository}" | base64 --decode | jq -r '.location')
                echo -e "\n\n    ###########################################################################"
                echo "     Processing repository:=============> ${repository_path} (ID: ${repository_id})"
                ...
                #   prj_location=$(echo "${prj_details}" | jq -r '.[].location')
                    echo -e "          Total Image(s) deleted in PROJECT: $del_location  are =======================> $del_local\n\n"
                    echo -e "Total Image(s) deleted in PROJECT: $del_location  are =======================> $del_local \n" >> $deleted_tags_proj
                    del_local=0
                fi
            fi
